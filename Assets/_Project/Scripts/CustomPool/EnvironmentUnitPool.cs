...
{
    public class EnvironmentUnitPool<T> where T : EnvironmentObject
    {
        private Transform _parent;
        private EnvironmentUnitFactory<T> _factory;
        private List<T> Pool { get; }

        public EnvironmentUnitPool(int prewarmObjects,Transform parent , EnvironmentUnitFactory<T> factory)
        {
            _factory = factory;
            _parent = parent;
            Pool = new List<T>();
